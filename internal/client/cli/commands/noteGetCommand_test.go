...
func TestNewNoteGetCommand_CommonChecks(t *testing.T) {
	...
	command := NewNoteGetCommand(stream, secretStorage, childCommand)
	command.ShowHelp()
	actualError := command.Invoke(ctx, keys)

	assert.Equal(t, "get", command.Name())
	assert.Equal(t, "get", command.FullName())
	assert.Equal(t, "get note secret", command.ShortDescription())
...
func TestNewNoteGetCommand_Invoke(t *testing.T) {
	ctx := context.Background()
	...
	tests := []struct {
	...
	}{
		{
			...
			expectedOutput: "\tsecretIdentity\t\t***\t\tsecretComment",
		}, {
			name:           "success_short_key_reveal",
			keys:           []string{"-id", secretIdentity, "-r"},
			expectedOutput: "\tsecretIdentity\t\tsecretText\t\tsecretComment",
		}, {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			currentSecret := secret.NewNoteSecret("secretText", secretIdentity, "secretComment")
